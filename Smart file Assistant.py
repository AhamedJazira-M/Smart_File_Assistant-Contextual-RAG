...
def setup_pipeline(file_obj):
    ...
    try:
        ...
        # ✅ Use a new temporary directory each upload to avoid readonly issues
        ...
        return f"❌ Error during pipeline setup: {str(e)}", ""
...
def answer_question(query):
    ...
    try:
        ...
        return f"⚠️ Error during question answering: {str(e)}", ""
...
theme = gr.themes.Base(
    ...
    body_background_fill="linear-gradient(135deg, #fce7f3, #bfdbfe, #ddd6fe, #e0e7ff)",  # rose → blue → violet → indigo
    ...
    button_primary_background_fill="linear-gradient(90deg, #ec4899, #6366f1, #3b82f6)",  # rose → indigo → blue
...
""") as demo:
    gr.Markdown("<h2 id='title'>📂 Smart File Assistant</h2>")
    gr.Markdown("Upload a file (CSV, TXT, PDF, DOCX) and ask questions. A vibrant, colorful assistant to help you understand your files 🌈")

    with gr.Row():
        with gr.Column(scale=1):
            file_upload = gr.File(label="📂 Upload File", file_types=[".csv", ".txt", ".pdf", ".docx"])
        ...
        with gr.Column(scale=2):
            ...
            context_output = gr.Textbox(label="📜 Used Context", lines=12, visible=False)
